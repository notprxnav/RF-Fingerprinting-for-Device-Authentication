%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  RF Device Fingerprinting with BPSK + Rayleigh Channel
%  PART 1  – BPSK signal generation with impairments
...
%% =============================================================
%   PART 1 — DEVICE SIGNAL GENERATION (BPSK)
% =============================================================
Nsym       = 200000;
M          = 2;          % BPSK
fs         = 1e6;
...
% =============================================================
clear; clc; close all;
load("RF_DeviceSignals_BPSK.mat");

fprintf("\n=== PART 2: BPSK + RAYLEIGH CHANNEL DATASET ===\n");

SNRdB        = 20;      % can change: 0, 5, 10, 20, 30
frameLen     = 1024;
...
for k = 1:numDevices
    ...
    for b = 1:numBlocks
        ...
        % Rayleigh flat fading coefficient
...
for ii = 1:2
    ...
    figure('Name',sprintf('BPSK Dev %d Clean vs Rayleigh',devID),'NumberTitle','off');
...
save("RF_Dataset_BPSK_Rayleigh.mat","X","Y","frameLen","numDevices","framesPerDevice","SNRdB");
fprintf("Saved: RF_Dataset_BPSK_Rayleigh.mat\n");

%% =============================================================
%   PART 3 — BPSK RAYLEIGH, 5-FOLD CV WITH FIXED BEST PARAMS
% =============================================================
clear; clc; close all;
load("RF_Dataset_BPSK_Rayleigh.mat");

fprintf("\n=== PART 3: BPSK + RAYLEIGH — 5-FOLD CV ===\n");
...
for modelID = 1:5
    ...
    for fold = 1:K
        ...
        switch modelID
            case 1
                % Shallow: F=80, FC=128, Drop=0.20, LR=1e-3
                layers = [
            ...
            case 2
                % Deep: F=80, FC=256, Drop=0.30, LR=5e-4
                layers = [
                    imageInputLayer([2 frameLen 1],"Normalization","none")
                    convolution2dLayer([1 7],80,"Padding","same")
                    batchNormalizationLayer
            ...
            case 3
                % ResNet: F=48, FC=128, Drop=0.30, LR=1e-3
                baseLayers = [
            ...
            case 4
                % Inception: F=32, FC=192, Drop=0.30, LR=1e-3
                F = 32;
                LG = layerGraph();
            ...
            case 5
                % DenseNet: G=32, FC=192, Drop=0.50, LR=1e-3
                G = 32;
                LG = layerGraph();
    ...
    title("Final Confusion Matrix — " + modelNames(modelID) + " (BPSK Rayleigh)");
...
fprintf("\n================== FINAL SUMMARY TABLE — BPSK RAYLEIGH ==================\n");
disp(Final);

writetable(Final,"BPSK_Rayleigh_5Fold_FixedBestParams_Results.csv");
fprintf("Saved: BPSK_Rayleigh_5Fold_FixedBestParams_Results.csv\n"); 